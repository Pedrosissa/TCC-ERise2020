                ...
                <div class="container-fluid">
                    <center>
                        ...
                        <form method="POST">
                            <div class="row">
                                <div class="col">
                                    <input type="text" class="form-control" style="width: 550px;" placeholder="Nome">
                                </div>
                            </div>
                            <br>
                            <button type="input" class="btn btn-primary btn-sm" name="pesquisar" value="pesquisar">Pesquisar</button><br>
                        </form>
                    </center><!-- Fim do form de pesquisa -->
                    <table class="table table-striped">
                        <!-- Inicio da tabela -->
                        <td scope="col"><b>ID</b></td>
                        <td scope="col"><b>Nome</b></td>
                        <td scope="col"><b>Representante</b></td>
                        <td scope="col"><b>Categorias</b></td>
                        <td scope="col"><b>Data de registro</b></td>
                        <td scope="col"><b>Deletar</b></td>
                        <td scope="col"><b>Editar</b></td>

                        <!-- Inicio do php -->
                        <!-- Busca dos dados da tebela "servicos" do banco "sgiaec" -->
                        <!-- Validação do click no botão da pesquisa -->
                        <?php
                        $pesquisa = filter_input(INPUT_POST, 'pesquisar', FILTER_SANITIZE_STRING);
                        if ($pesquisa) {
                            $nome = filter_input(INPUT_POST, 'nome', FILTER_SANITIZE_STRING);
                            $result_pesquisa = "SELECT * FROM loja WHERE nome LIKE '%$nome%'";
                            $resultado = mysqli_query($conn, $result_pesquisa);
                            echo "<br>";
                            while ($row_pesq = mysqli_fetch_assoc($resultado)) {
                                echo "<tr>";
                                echo "<td>" . $row_pesq['idloja'] . "</td>";
                                echo "<td>" . $row_pesq['nome'] . "</td>";
                                echo "<td>" . $row_pesq['dono'] . "</td>";
                                echo "<td>" . $row_pesq['categorias'] . "</td>";
                                echo "<td>" . $row_pesq['dataregistro'] . "</td>";
                                echo "<td><a href='../API/apagaruser.php?id=" . $row_pesq['idloja'] . "'>
                                <button type='button' class='btn btn-danger'>
                                Apagar</button></a></td>";
                                echo "<td><a href='editar.php?id=" . $row_pesq['idloja'] . "'>
                                <button type='button' class='btn btn-primary'>
                                Editar</button></a></td>";
